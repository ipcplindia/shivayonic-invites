...
export async function GET(_: Request, route: { params: Promise<{ slug: string }> }) {
  ...
  const product = await prisma.publicProduct.findFirst({ where: { slug, status: "PUBLISHED", organizationId: await getPublicOrganizationId() }, include: catalogueProductInclude });
