...
vi.mock("./razorpay-provider", async importOriginal => ({ ...await importOriginal<typeof import("./razorpay-provider")>(), razorpayConfig: mocks.config, createOrder: mocks.createOrder, getPayment: mocks.getPayment, verifyCheckout: mocks.verify, verifyWebhook: mocks.webhook }));
...
describe("Razorpay payment boundaries", () => {
  beforeEach(() => { vi.resetAllMocks(); vi.stubEnv("PAYMENTS_ENABLED", "true"); vi.stubEnv("RAZORPAY_MODE", "TEST"); vi.stubEnv("VERCEL_ENV", "preview"); mocks.config.mockReturnValue({ key: "rzp_test_fixture", mode: "TEST" }); mocks.org.mockResolvedValue("canonical"); });
  afterEach(() => vi.unstubAllEnvs());
  function intent() { const capability = createPaymentCapability("intent"); const row = { id: "intent", organizationId: "canonical", status: "READY", currency: "INR", amountMinor: 100n, paymentAccessHash: capability.hash, paymentAccessExpiresAt: capability.expiresAt, providerEnvironment: "TEST", providerOrderId: null }; mocks.find.mockResolvedValue(row); return { row, capability }; }
