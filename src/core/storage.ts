...
export interface ObjectStorage {
  ...
  createDownloadTarget(input: { storageKey: string }): Promise<DownloadTarget | null>;
  putObject(input: { storageKey: string; body: globalThis.ReadableStream<Uint8Array>; maxBytes: number }): Promise<{ sizeBytes: number }>;
  getObject(input: { storageKey: string }): Promise<{ body: globalThis.ReadableStream<Uint8Array>; sizeBytes?: number }>;
...
export class LocalObjectStorage implements ObjectStorage {
  ...
  async createDownloadTarget(input: { storageKey: string }): Promise<DownloadTarget | null> { void input; return null; }
  ...
  async getObject({ storageKey }: { storageKey: string }) {
    const filePath = this.toPath(storageKey);
    const details = await stat(filePath);
    return { body: Readable.toWeb(createReadStream(filePath)) as unknown as globalThis.ReadableStream<Uint8Array>, sizeBytes: details.size };
  }
...
export class S3ObjectStorage implements ObjectStorage {
  ...
  async createDownloadTarget({ storageKey }: { storageKey: string }) {
    const expiresIn = 15 * 60;
    return { url: await getSignedUrl(this.client, new GetObjectCommand({ Bucket: this.bucket, Key: storageKey }), { expiresIn }), expiresAt: new Date(Date.now() + expiresIn * 1_000) };
  }
  ...
  async getObject({ storageKey }: { storageKey: string }) {
    ...
    return { body: result.Body.transformToWebStream() as unknown as globalThis.ReadableStream<Uint8Array>, sizeBytes: result.ContentLength };
  }
